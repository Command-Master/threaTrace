...
def validate():
    ...
    data, feature_num, label_num, adj, adj2, nodeA, _nodeA, _neibor = MyDatasetA(output_path, path, 0)
    dataset = TestDatasetA(data, args.scene)
    data = dataset[0]
    print(data)
    ...
    while (1):
        ...
        model_path = output_path + 'model_' + str(out_loop)
        ...
        if (recall > 0.8) and (precision > 0.7):
            while 1:
                ...
                model_path = output_path + 'model_' + str(out_loop)
                if not osp.exists(model_path): break
                os.system(f'rm {output_path}model_' + str(out_loop))
                os.system(f'rm {output_path}tn_feature_label_' + str(graphId) + '_' + str(out_loop) + '.txt')
                os.system(f'rm {output_path}fp_feature_label_' + str(graphId) + '_' + str(out_loop) + '.txt')
            return 1
        if recall <= 0.8:
            return 0
        for j in tn:
...
def train_pro():
    ...
    data1, feature_num, label_num, adj, adj2 = MyDataset(output_path, path, 0)
    dataset = TestDataset(data1, args.scene)
    data = dataset[0]
    print(data)
    ...
    while 1:
        ...
        if len(tn) > 0:
            ...
            fw = open(output_path + 'fp_feature_label_' + str(graphId) + '_' + str(loop_num) + '.txt', 'w')
            x_list = data.x[fp]
            y_list = data.y[fp]
            print(len(x_list))
            ...
            fw = open(output_path + 'tn_feature_label_' + str(graphId) + '_' + str(loop_num) + '.txt', 'w')
            x_list = data.x[tn]
            y_list = data.y[tn]
            print(len(x_list))
            ...
            for i in range(len(y_list)):
                ...
                fw.write('\n')
            fw.close()
            torch.save(model.state_dict(), output_path + 'model_' + str(loop_num))
            loop_num += 1
            if len(fp) == 0: break
        auc = 0
...
def main():
    global b_size, args, thre, output_path
    ...
    output_path = f'../outputs/{args.scene}_train/'
    b_size = 5000
    thre = thre_map[args.scene]
    os.system('cp ../groundtruth/' + args.scene + f'.txt {output_path}groundtruth_uuid.txt')
    while 1:
        train_pro()
        flag = validate()
        if flag == 1:
            ...
            os.system(f'rm {output_path}model_*')
            os.system(f'rm {output_path}tn_feature_label_*')
            os.system(f'rm {output_path}fp_feature_label_*')
