...
class TestDatasetA(InMemoryDataset):
    def __init__(self, data_list, scene):
        super(TestDatasetA, self).__init__(f'/tmp/TestDataset_{scene}')
...
def MyDatasetA(output_path, path, model):
    graphId = model
    feature_num = 0
    label_num = 0
    f_feature = open(output_path + 'feature.txt', 'r')
    feature_map = {}
    for i in f_feature:
        temp = i.strip('\n').split('\t')
    ...
    f_label = open(output_path + 'label.txt', 'r')
    label_map = {}
    for i in f_label:
        temp = i.strip('\n').split('\t')
    ...
    f_gt = open(output_path + 'groundtruth_uuid.txt', 'r')
    ground_truth = {}
    for line in f_gt:
    ...
    adj2 = {}
    data_thre = 1000000
    fw = open(output_path + 'groundtruth_nodeId.txt', 'w')
    fw2 = open(output_path + 'id_to_uuid.txt', 'w')
    nodeId_map = {}
    cnt = 0
    nodeA = []
    for i in range(1):
